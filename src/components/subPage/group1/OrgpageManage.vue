  ...
  <div class="orgpage">
    ...
    <el-container class="container-css">
      <el-aside style="background-color: #ffffff; width: 30%;height: 100%">
        <el-tree :data="dataArray" :props="defaultProps"
                 @node-click="handleNodeClick" node-key="id" :default-expanded-keys="keys">
          <span slot-scope="{ node, data }">
        <span>{{node.label }}</span>
        <span>
          <el-button @click.stop="creatOrg(data)" type="primary" size="mini" icon="el-icon-circle-plus-outline" circle
                     title="添加部门信息"></el-button>
          <el-button size="mini" v-show="node.data.parentid!='0'" type="danger" icon="el-icon-delete" circle
                     title="删除部门信息" @click.stop="deleteOrgInfo(node,data)"></el-button>
        </span>
      </span>
        </el-tree>
      </el-aside>
        ...
        <div class="org-edit" v-if="orgDiv">
          ...
          <div class="dep-content">
            <el-tabs v-model="activeName">
              <el-tab-pane label="基本信息" name="first">
                <el-form ref="defaultProps" :model="defaultProps" label-width="80px">
                  ...
                  <el-form-item label="上级部门">
                    <el-input v-model="defaultProps.parentname" readonly style="color: #625C65;cursor: default;outline: none;"></el-input>
                  </el-form-item>
                  <el-form-item label="部门描述">
                    <el-input type="textarea" v-model="defaultProps.remark"
                              placeholder="请输入部门职责及描述，不得超过200个汉字"></el-input>
                  </el-form-item>
              ...
              <el-tab-pane label="用户信息" name="second" v-if="tabShow">
...
<script>
  ...
  import {deleteOrgInfo} from "@/api/sysOrg"
  import {addOrgInfo} from "@/api/sysOrg"
  import {modifyOrgInfo} from "@/api/sysOrg"
  ...
  export default {
    ...
    methods: {
      handleNodeClick(node) {
        ...
        this.orgDiv = true;
        this.viewName = "编辑部门信息 / 【" + node.label + "】";
        this.node = node;
      ...
      changeAdress(row, column) { //显示当前部门名称
    ...
    mounted() {
      initOrgInfos().then(response => {
        if (response.status == 200) {
          ...
          let parent = [];
          this.dataArray = toTreeData(treeList);
        }
      })
